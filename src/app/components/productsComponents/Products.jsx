...
const FetchProduct = async ({ Category }) => {
  ...
  return products.map((product) => (
    <div
      className="relative z-10 group h-[420px] w-[235px] overflow-hidden rounded-sm shadow"
      key={product.id}
    >
      <Link href={`/detail/${product.id}`} className="relative z-10 ">
        ...
        >
          <div className="grid grid-cols-1 [&>*]:col-start-1 [&>*]:row-start-1 col-start-1! col-span-full overflow-hidden">
            <h1 className="text-primary p-4 text-2xl font-bold absolute uppercase z-10 italic">
              {product.category}
            </h1>
            <Image
              className="group-hover:scale-110 transition-all ease-in-out duration-300 h-full w-full object-contain bg-(--blue)/60"
              loading="eager"
          ...
          <div className="grid grid-cols-[1fr_auto] gap-4 py-4 min-w-0">
            <div className="min-w-0">
              <h2 className=" text-[13px] font-semibold h-[45px]">
